...
void SenderY::genStatBlk(uint8_t blkBuf[BLK_SZ_CRC], const char* fileName)
{
    ...
    //memset(&buffer[strlen(subFileName.c_str())+2], myCrc16ns, 1); //strlen(crcNum.c_str()));
    memcpy(&buffer[strlen(subFileName.c_str())+2], fileSizeChar.c_str(), strlen(fileSizeChar.c_str()));
    memset(&buffer[CHUNK_SZ-1], myCrc16ns, 1);
